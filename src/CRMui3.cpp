...
void CRMui3::http() {
  ...
  server.on("/ui", HTTP_POST, [this](AsyncWebServerRequest * request) {
    ...
    _buf = String();
    _replauseString = String();
  });

    server.on("/cfg", HTTP_POST, [this](AsyncWebServerRequest * request) {
    if (_AuthenticateStatus && !request->authenticate(_WebAuthLogin.c_str(), _WebAuthPass.c_str()))
      return request->requestAuthentication();
    String _bufCFG = String();
    serializeJson(cfg, _bufCFG);
    if (_bufCFG.length() > 7840) request->send_P(200, F("text/plain"), _bufCFG.c_str());
    else request->send(200, F("text/plain"), _bufCFG);
  });
