...
void CRMui3::select(Select_C item) {
  if (var(item.id) == F("null") and item.type != INPUT_SELECT_PIECE_UP and item.type < 50) var(item.id, item.defaultValue);
  if (_start) return;
...
void CRMui3::input(Input item) {
  if (item.type != INPUT_BUTTON && item.type != INPUT_WIFI && var(item.id) == F("null") && item.type < 50)
...
void CRMui3::card(Card item) {
  if (item.type == CARD_CHECKBOX && var(item.id) == F("null") && item.type < 50)
    var(item.id, item.defaultValue);
  if (_start) return;
  if (_buf.endsWith("]")) _buf[_buf.length() - 1] = ',';
  _buf += String() + F("{\"t\":") + String(item.type);
...
void CRMui3::addReplause(const size_t &namber, const String &var) {
  if (_start) return;
  if (_replauseString != "") _replauseString += ",";
  _replauseString += String() + F("\"$") + String(namber) + F("\":\"") + var + F("\"");
